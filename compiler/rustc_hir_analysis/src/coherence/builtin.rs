...
fn visit_implementation_of_dispatch_from_dyn(checker: &Checker<'_>) -> Result<(), ErrorGuaranteed> {
    ...
    match (source.kind(), target.kind()) {
        (&ty::Refined(_, refinement_a), &ty::Refined(_, refinement_b)) => {
            if refinement_a != refinement_b {
                return Err(tcx.dcx().emit_err(diagnostics::CoerceSameRefinement {
                    span,
                    trait_name,
                    refinement_a: refinement_a.to_string(),
                    refinement_b: refinement_b.to_string(),
                }));
            }
            Ok(())
        }
...
pub(crate) fn coerce_unsized_info<'tcx>(
    ...
    let (source, target, trait_def_id, kind, field_span) = match (source.kind(), target.kind()) {
        (&ty::Refined(ty_a, refinement_a), &ty::Refined(ty_b, refinement_b)) => {
            if refinement_a != refinement_b {
                return Err(tcx.dcx().emit_err(diagnostics::CoerceSameRefinement {
                    span,
                    trait_name,
